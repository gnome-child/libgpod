...
#define ITDB_SPLACTION_LAST_LUNARCYCLE_VALUE                                                       \
    2551443 // a "lunar cycle" is the time it takes the moon to circle the earth
#define ITDB_SPLACTION_LAST_SIDEREAL_DAY                                                           \
    86164 // a "sidereal day" is time in one revolution of earth on its axis
#define ITDB_SPLACTION_LAST_SWATCH_BEAT                                                            \
...
#define ITDB_SPLACTION_LAST_SOLAR_YEAR                                                             \
    31556926 // a "solar year" is the time it takes the earth to go around the sun
#define ITDB_SPLACTION_LAST_SIDEREAL_YEAR                                                          \
    31558150 // a "sidereal year" is the time it takes the earth to reach the same point in space
...
gchar* itdb_cp_get_dest_filename(Itdb_Track* track, const gchar* mountpoint, const gchar* filename,
                                 GError** error);
gboolean itdb_cp(const gchar* from_file, const gchar* to_file, GError** error);
Itdb_Track* itdb_cp_finalize(Itdb_Track* track, const gchar* mountpoint, const gchar* dest_filename,
                             GError** error);
...
gboolean itdb_track_set_thumbnails_from_data(Itdb_Track* track, const guchar* image_data,
...
Itdb_Artwork* itdb_photodb_add_photo(Itdb_PhotoDB* db, const gchar* filename, gint position,
                                     gint rotation, GError** error);
Itdb_Artwork* itdb_photodb_add_photo_from_data(Itdb_PhotoDB* db, const guchar* image_data,
                                               gsize image_data_len, gint position, gint rotation,
                                               GError** error);
Itdb_Artwork* itdb_photodb_add_photo_from_pixbuf(Itdb_PhotoDB* db, gpointer pixbuf, gint position,
                                                 gint rotation, GError** error);
void itdb_photodb_photoalbum_add_photo(Itdb_PhotoDB* db, Itdb_PhotoAlbum* album,
                                       Itdb_Artwork* photo, gint position);
Itdb_PhotoAlbum* itdb_photodb_photoalbum_create(Itdb_PhotoDB* db, const gchar* albumname, gint pos);
...
void itdb_photodb_remove_photo(Itdb_PhotoDB* db, Itdb_PhotoAlbum* album, Itdb_Artwork* photo);
void itdb_photodb_photoalbum_remove(Itdb_PhotoDB* db, Itdb_PhotoAlbum* album, gboolean remove_pics);
...
gboolean itdb_artwork_set_thumbnail(Itdb_Artwork* artwork, const gchar* filename, gint rotation,
                                    GError** error);
gboolean itdb_artwork_set_thumbnail_from_data(Itdb_Artwork* artwork, const guchar* image_data,
                                              gsize image_data_len, gint rotation, GError** error);
gboolean itdb_artwork_set_thumbnail_from_pixbuf(Itdb_Artwork* artwork, gpointer pixbuf,
                                                gint rotation, GError** error);
...
gpointer itdb_artwork_get_pixbuf(Itdb_Device* device, Itdb_Artwork* artwork, gint width,
                                 gint height);

/* itdb_thumb_... */
Itdb_Thumb* itdb_thumb_duplicate(Itdb_Thumb* thumb);
gpointer itdb_thumb_to_pixbuf_at_size(Itdb_Device* device, Itdb_Thumb* thumb, gint width,
                                      gint height);
...
gboolean itdb_chapterdata_add_chapter(Itdb_Chapterdata* chapterdata, guint32 startpos,
                                      gchar* chaptertitle);
...
gboolean itdb_init_ipod(const gchar* mountpoint, const gchar* model_number, const gchar* ipod_name,
                        GError** error);
