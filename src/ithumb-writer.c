...
static guint get_aligned_width(const Itdb_ArtworkFormat* img_info, gsize pixel_size) {
    guint width;
...
static guint16* pack_RGB_565(GdkPixbuf* pixbuf, const Itdb_ArtworkFormat* img_info,
                             gint horizontal_padding, gint vertical_padding, guint32* thumb_size) {
    guchar* pixels;
    guint16* result;
    gint row_stride;
    gint channels;
    ...
    g_object_get(G_OBJECT(pixbuf), "rowstride", &row_stride, "n-channels", &channels, "height",
                 &height, "width", &width, "pixels", &pixels, NULL);
    ...
    *thumb_size = dest_width * img_info->height * 2;
    ...
    for (h = 0; h < vertical_padding; h++) {
        ...
        for (w = 0; w < dest_width; w++) {
            result[line + w] = get_RGB_565_pixel(img_info->back_color, byte_order);
        }
    }
    ...
    for (h = 0; h < height; h++) {
        ...
        for (w = 0; w < dest_width; w++) {
            ...
            if (w < horizontal_padding) {
                packed_pixel = get_RGB_565_pixel(img_info->back_color, byte_order);
            } else if (w >= horizontal_padding + width) {
                packed_pixel = get_RGB_565_pixel(img_info->back_color, byte_order);
            } else {
                guchar* cur_pixel;

                cur_pixel = &pixels[h * row_stride + (w - horizontal_padding) * channels];
...
static guint16 get_RGB_555_pixel(const guchar* pixel, gint byte_order, gboolean has_alpha) {
    gint r;
    gint g;
    gint b;
    gint a;
...
static guint16* pack_RGB_555(GdkPixbuf* pixbuf, const Itdb_ArtworkFormat* img_info,
                             gint horizontal_padding, gint vertical_padding, guint32* thumb_size) {
    guchar* pixels;
    guint16* result;
    gint row_stride;
    ...
    *thumb_size = dest_width * img_info->height * 2;
    ...
    for (h = 0; h < vertical_padding; h++) {
        ...
        for (w = 0; w < dest_width; w++) {
            result[line + w] = get_RGB_555_pixel(img_info->back_color, byte_order, TRUE);
        }
    }
    ...
    for (h = 0; h < height; h++) {
        ...
        for (w = 0; w < dest_width; w++) {
            guint16 packed_pixel;
            if (w < horizontal_padding) {
                packed_pixel = get_RGB_555_pixel(img_info->back_color, byte_order, TRUE);
            } else if (w >= horizontal_padding + width) {
                packed_pixel = get_RGB_555_pixel(img_info->back_color, byte_order, TRUE);
            } else {
                guchar* cur_pixel;
                ...
                cur_pixel = &pixels[h * row_stride + (w - horizontal_padding) * channels];
                packed_pixel = get_RGB_555_pixel(cur_pixel, byte_order, FALSE);
            }
            result[line + w] = packed_pixel;
        }
    }
...
static guint16 get_RGB_888_pixel(const guchar* pixel, gint byte_order, gboolean has_alpha) {
    gint r;
    gint g;
    gint b;
    gint a;
...
static guint16* pack_RGB_888(GdkPixbuf* pixbuf, const Itdb_ArtworkFormat* img_info,
                             gint horizontal_padding, gint vertical_padding, guint32* thumb_size) {
    guchar* pixels;
    guint32* result;
    gint row_stride;
    ...
    *thumb_size = img_info->width * img_info->height * 4;
    ...
    for (h = 0; h < vertical_padding; h++) {
        ...
        for (w = 0; w < img_info->width; w++) {
            result[line + w] = get_RGB_888_pixel(img_info->back_color, byte_order, TRUE);
        }
    }
    ...
    for (h = 0; h < height; h++) {
        ...
        for (w = 0; w < img_info->width; w++) {
            guint16 packed_pixel;
            if (w < horizontal_padding) {
                packed_pixel = get_RGB_888_pixel(img_info->back_color, byte_order, TRUE);
            } else if (w >= horizontal_padding + width) {
                packed_pixel = get_RGB_888_pixel(img_info->back_color, byte_order, TRUE);
            } else {
                guchar* cur_pixel;
                ...
                cur_pixel = &pixels[h * row_stride + (w - horizontal_padding) * channels];
                packed_pixel = get_RGB_888_pixel(cur_pixel, byte_order, FALSE);
            }
            result[line + w] = packed_pixel;
        }
    }
...
static guint16* derange_pixels(guint16* pixels_s, guint16* pixels_d, gint width, gint height,
                               gint row_stride) {
    ...
    if (width == 1) {
        *pixels_s = *pixels_d;
        ...
        derange_pixels(pixels_s + 0, pixels_d + 0 + 0, width / 2, height / 2, row_stride);
        derange_pixels(pixels_s + (width / 2) * (height / 2),
                       pixels_d + (height / 2) * row_stride + 0, width / 2, height / 2, row_stride);
        derange_pixels(pixels_s + 2 * (width / 2) * (height / 2), pixels_d + width / 2, width / 2,
                       height / 2, row_stride);
        derange_pixels(pixels_s + 3 * (width / 2) * (height / 2),
                       pixels_d + (height / 2) * row_stride + width / 2, width / 2, height / 2,
                       row_stride);
    }
...
static guint16* pack_rec_RGB_555(GdkPixbuf* pixbuf, const Itdb_ArtworkFormat* img_info,
                                 gint horizontal_padding, gint vertical_padding,
                                 guint32* thumb_size) {
    guint16* pixels;
    guint16* deranged_pixels = NULL;

    pixels = pack_RGB_555(pixbuf, img_info, horizontal_padding, vertical_padding, thumb_size);
    ...
    if (pixels) {
        ...
        deranged_pixels =
            derange_pixels(NULL, pixels, img_info->width, img_info->height, row_stride);
        g_free(pixels);
    }
...
static guchar* pack_I420(GdkPixbuf* orig_pixbuf, const Itdb_ArtworkFormat* img_info,
                         gint horizontal_padding, gint vertical_padding, guint32* thumb_size) {
    GdkPixbuf* pixbuf;
    gint width, height;
    ...
    g_object_get(G_OBJECT(orig_pixbuf), "height", &orig_height, "width", &orig_width, NULL);

    /* copy into new pixmap with padding applied */
    pixbuf =
        gdk_pixbuf_new(GDK_COLORSPACE_RGB, gdk_pixbuf_get_has_alpha(orig_pixbuf), 8, width, height);
    gdk_pixbuf_copy_area(orig_pixbuf, 0, 0, orig_width, orig_height, pixbuf, horizontal_padding,
                         vertical_padding);

    g_object_get(G_OBJECT(pixbuf), "rowstride", &rowstride, "pixels", &pixels, NULL);
 ...
 * (www.gpixpod.org) */
static guchar* pack_UYVY(GdkPixbuf* orig_pixbuf, const Itdb_ArtworkFormat* img_info,
                         gint horizontal_padding, gint vertical_padding, guint32* thumb_size) {
    GdkPixbuf* pixbuf;
    guchar *pixels, *yuvdata;
    gint width;
    ...
    while (h < height) {
        ...
        if ((h % 2) == 0) {
            ...
            while (w < width) {
                ...
                yuvdata[halfyuv + z2 + 2] =
                    ((r2 * 112 + g2 * -94 + b2 * -18 + 128) >> 8) + 128;                      /*V1*/
...
static char* get_ithmb_filename(iThumbWriter* writer) {
    ...
    case DB_TYPE_ITUNES:
        return g_strdup_printf(":F%d_%d.ithmb", writer->img_info->format_id,
                               writer->current_file_index);
    case DB_TYPE_PHOTO:
        return g_strdup_printf(":Thumbs:F%d_%d.ithmb", writer->img_info->format_id,
                               writer->current_file_index);
    }
    g_return_val_if_reached(NULL);
}

static char* ipod_image_get_ithmb_filename(const char* ithmb_dir, gint format_id, gint index) {
    gchar *filename, *buf;
...
static GdkPixbuf* ithumb_writer_handle_rotation(GdkPixbuf* pixbuf, guint* rotation) {
...
/* On the iPhone, thumbnails are presented as squares in a grid.
   ...
   scaled. */
static GdkPixbuf* ithumb_writer_scale_and_crop(GdkPixbuf* input_pixbuf, gint width, gint height,
                                               gboolean crop) {
    ...
    g_object_get(G_OBJECT(input_pixbuf), "width", &input_width, "height", &input_height, NULL);
    ...
    if (!crop) {
        ...
        return gdk_pixbuf_scale_simple(input_pixbuf, scaled_width, scaled_height,
                                       GDK_INTERP_BILINEAR);
    } else {
       ...
       offset. */
        ...
        g_assert(round(scaled_width) == width || round(scaled_height) == height);

        output_pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, gdk_pixbuf_get_has_alpha(input_pixbuf),
                                       8, width + border_width, height + border_width);
        gdk_pixbuf_fill(output_pixbuf, 0xffffffff);

        gdk_pixbuf_scale(input_pixbuf, output_pixbuf, 0, 0, /* dest x, dest y */
                         width,                             /* dest width */
                         height,                            /* dest height */
...
static GdkPixbuf* ithumb_writer_handle_pixbuf_transform(iThumbWriter* writer, GdkPixbuf* pixbuf,
                                                        guint rotation) {
    ...
    scaled_pixbuf = ithumb_writer_scale_and_crop(rotated_pixbuf, writer->img_info->width,
                                                 writer->img_info->height, writer->img_info->crop);
...
static void* pack_thumbnail(iThumbWriter* writer, Itdb_Thumb_Ipod_Item* thumb, GdkPixbuf* pixbuf) {
    typedef void* (*PackerFunc)(GdkPixbuf* pixbuf, const Itdb_ArtworkFormat* img_info,
                                gint horizontal_padding, gint vertical_padding,
                                guint32* thumb_size);
    struct Packer {
    ...
    guint i;
    const struct Packer packers[] = {{THUMB_FORMAT_RGB565_LE_90, (PackerFunc)pack_RGB_565},
    ...
    return packers[i].packer(pixbuf, writer->img_info, thumb->horizontal_padding,
                             thumb->vertical_padding, &thumb->size);
}

static gboolean write_pixels(iThumbWriter* writer, Itdb_Thumb_Ipod_Item* thumb, void* pixels) {
    if (pixels == NULL) {
        return FALSE;
    }
...
static Itdb_Thumb_Ipod_Item* ithumb_writer_write_thumbnail(iThumbWriter* writer,
                                                           Itdb_Thumb* thumb) {
    ...
    */
    if (thumb->data_type == ITDB_THUMB_TYPE_FILE) {
        ...
        pixbuf = pixbuf_from_image_data(thumb_mem->image_data, thumb_mem->image_data_len);
...
#pragma GCC diagnostic pop
        if (!pixbuf) {
            ...
            pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, FALSE, 8, writer->img_info->width,
    ...
    scaled_pixbuf =
        ithumb_writer_handle_pixbuf_transform(writer, pixbuf, itdb_thumb_get_rotation(thumb));
    g_object_unref(pixbuf);
    pixbuf = NULL;
    ...
    /* !! cannot write directly to &thumb->width/height because
       ...
       gint16 !! */
    g_object_get(G_OBJECT(scaled_pixbuf), "width", &width, "height", &height, NULL);
...
static gboolean ithumb_writer_update(iThumbWriter* writer) {
    while ((writer->f == NULL) || (writer->cur_offset >= ITHUMB_MAX_SIZE)) {
        ...
        writer->filename = ipod_image_get_ithmb_filename(
            writer->thumbs_dir, writer->img_info->format_id, writer->current_file_index);
...
static void write_thumbnail(iThumbWriter* writer, Itdb_Artwork* artwork,
    ...
    if (ithumb_writer_update(writer)) {
        ...
        item = ithumb_writer_write_thumbnail(writer, artwork->thumbnail);
        if (item != NULL) {
...
static void ithumb_writer_free(iThumbWriter* writer) {
    g_return_if_fail(writer != NULL);
    if (writer->f) {
        fclose(writer->f);
        if (writer->filename && (writer->cur_offset == 0)) { /* Remove empty file */
...
static iThumbWriter* ithumb_writer_new(const char* thumbs_dir, const Itdb_ArtworkFormat* info,
                                       DbType db_type, guint byte_order) {
    iThumbWriter* writer;
...
static gboolean ithumb_rearrange_thumbnail_file(gpointer _key, gpointer _thumbs,
                                                gpointer _user_data) {
    ...
    if (thumbs == NULL) { /* no thumbnails for this file --> remove altogether */
        ...
        if (thumb->offset > offset) {
            ...
            while (first_thumb->offset == first_offset) {
                ...
                if (gl != first_gl) {
                    ...
                    /* Insert /behind/ gl */
                    thumbs = g_list_insert_before(thumbs, gl->next, first_thumb);
...
*/
static gboolean ithmb_rearrange_existing_thumbnails(const gchar* thumbs_dir, Itdb_DB* db,
    ...
    filenamehash = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
    ...
    case DB_TYPE_ITUNES:
        for (gl = db_get_itunesdb(db)->tracks; gl; gl = gl->next) {
            ...
            if (thumb->data_type == ITDB_THUMB_TYPE_IPOD) {
                ...
                if (item) {
                    filename = itdb_thumb_ipod_get_filename(db_get_device(db), item);
                    if (filename) {
    ...
    for (i = 0; i < 50; ++i) {
        filename = ipod_image_get_ithmb_filename(thumbs_dir, info->format_id, i);
        if (g_file_test(filename, G_FILE_TEST_EXISTS)) {
            if (g_hash_table_lookup(filenamehash, filename) == NULL) {
                g_hash_table_insert(filenamehash, g_strdup(filename), NULL);
            }
        }
        g_free(filename);
    }
    ...
    g_hash_table_foreach_remove(filenamehash, ithumb_rearrange_thumbnail_file, &result);
...
G_GNUC_INTERNAL int itdb_write_ithumb_files(Itdb_DB* db) {
    ...
    for (it = formats; it != NULL; it = it->next) {
        ...
        writer = ithumb_writer_new(thumbs_dir, format, db->db_type, device->byte_order);
        if (writer != NULL) {
        ...
        for (it = db_get_itunesdb(db)->tracks; it != NULL; it = it->next) {
            ...
            if (type != ITDB_THUMB_TYPE_IPOD) {
                ...
                for (itw = writers; itw != NULL; itw = itw->next) {
                    write_thumbnail(itw->data, track->artwork, thumb_ipod);
                }
