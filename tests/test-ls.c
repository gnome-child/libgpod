...
static void display_recently_played(Itdb_iTunesDB* db) {
    ...
    for (it = mpl->members; it != NULL; it = it->next) {
        ...
        if (track->recent_playcount != 0) {
            ...
            strftime(date, sizeof(date), "%F %H:%M:%S", gmtime(&track_time));
            g_print("\tUTC: %s\n", date);
            strftime(date, sizeof(date), "%F %H:%M:%S", localtime(&track_time));
            g_print("\tlocal: %s\n", date);
...
static void display_track(Itdb_Track* track, const char* prefix) {
    g_print("%s%s - %s - %s\n", prefix, track->artist ? track->artist : "(null)",
            track->album ? track->album : "(null)", track->title ? track->title : "(null)");
    g_print("%s\t%s\n", prefix, track->ipod_path);
}

static void display_playlist(Itdb_Playlist* playlist, const char* prefix) {
    char* track_prefix;
    GList* it;
